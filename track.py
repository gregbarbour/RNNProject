...
class Track:
    ...
    def __init__(self, particle, charge):
        ...
        self.qOverP = charge * self.addGaussianError(1./particle.magp, 1e-5) # now doing abs err
        ...
        if true_d0 == 0.: # should instead be something like isPrimaryTrack...
            self.d0 = self.addGaussianError(true_d0, 1e-5) # this is very small, 1um, not realistic
            self.z0 = self.addGaussianError(true_z0, 1e-5)
        else:
            self.d0 = self.addGaussianError(true_d0, 1e-5) # 1% error # now doing abs errs
            self.z0 = self.addGaussianError(true_z0, 1e-5)
    ...
    def addGaussianError(self, parameter, std=None):
        ...
        if std is None:
            ...
            std = abs(parameter / 10000)  #0.01% is min err # essentially a 1% error, absolute value taken
